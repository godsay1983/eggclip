...
“设置 → 常规 → 语言”支持“跟随系统”“简体中文”和“English”。切换后主面板、动态状态、对话框、错误、托盘菜单和 tooltip 会立即使用同一种语言，重启后保持选择；跟随不受支持的系统语言时回退为 English。语言偏好只保存在本机，不参与协议同步。程序生成的空间和设备名称按当前语言显示，用户自定义名称保持原文。
...
协调端同时连接多台 Windows、手机或平板时，会把首次收到的同空间 `ITEM_LIVE` 直接安全转发给其他在线会话，并排除来源连接。转发给每个目标的帧分别使用对应会话密钥加密；有界去重缓存只保存事件身份和 HMAC 摘要，不保存正文。重复事件只返回确认，不再写入或转发；`ITEM_BATCH` 仍只补历史。成员端尚未取得完整成员清单时，可以显示由已认证协调端中继的实时文本，但不会伪造可信设备记录或把未知来源写入本地历史。

Windows 的服务端方向和客户端方向统一处理 `SYNC_HEADS`、`REQUEST_RANGE`、`ITEM_BATCH` 与 `ITEM_ACK`。重连时只把缺失事件补入加密历史，`ITEM_BATCH` 不写系统剪贴板也不实时转发；关闭同步、自动接收或历史后不会发起无意义的历史请求。协调端移除成员会立即关闭旧会话、标记撤销并轮换空间密钥；在线成员即时接收新密钥，离线成员可信重连时补领。轮换会在两端原子清理旧密钥绑定的历史和同步头。

同步空间卡片会区分“协调端”和“成员端”。只有协调端可以生成邀请、移除设备和轮换全局空间密钥；成员端可以执行“离开空间”，清除该空间的可信路由、本地历史和密钥。离开后必须使用新邀请才能再次加入；如果本机没有其他空间，会自动创建一个由本机管理的默认空间。

设备页提供“添加设备”和“加入另一台电脑”两个入口。加入电脑时粘贴邀请并核对确认码，界面只展示设备短指纹、空间摘要和脱敏候选地址；默认按用户选择的优先地址依次尝试全部候选，手动 IPv4/端口位于高级区域。邀请过期、已使用、网络不可达、身份不匹配、认证失败、密钥保存失败和数据库失败会给出不同提示。完成、取消或关闭后，前端和 Rust 临时加入状态都不再保留完整邀请。新配对设备默认命名为“EggClip 设备 #短指纹”，可在可信设备卡片重命名；未认证 POC 只在高级诊断中显示，不进入可信设备列表。
...
该检查校验中英文 key、占位参数、HarmonyOS 资源和用户界面硬编码，不读取或输出剪贴板正文、邀请或密钥。
