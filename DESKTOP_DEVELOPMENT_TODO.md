...
- [x] 生成二维码内容和可复制邀请字符串。
  ...
  - [x] 桌面端已在本地生成邀请二维码 SVG 并展示在设置弹层邀请卡中；二维码只在内存/UI 中呈现，不记录邀请明文。
...
- [ ] mDNS 失败时尝试最近成功地址。
  - [x] POC 阶段已在手动连接卡持久化最近主动连接成功地址，支持重启后回填并连接；该地址明确标记为非可信设备信息。
- [ ] 手动 IP 始终作为诊断回退入口。
...
- [x] 当前剪贴板卡片显示预览、来源和时间。
  ...
  - [x] 当前剪贴板卡片增加发送状态：区分本机记录、等待连接、发送中、已发送、发送失败和同步暂停，不把 POC 连接误展示为正式同步。
  - [x] 主面板外层取消常驻滚动条，改为固定外壳和最近历史/设置弹层等局部滚动；滚动条样式适配浅色、深色和跟随系统主题。
- [ ] 历史列表支持复制、删除单条和清空。
...
- [x] 设备 chips 显示在线、连接中、离线和认证失败。
  ...
  - [x] 设置弹层设备区域已将 POC peer 展示为“实验连接”，显示短指纹占位、最后在线、端点和未配对说明，避免误认为 trusted device。
...
- [ ] 显示公钥短指纹和最后在线时间。
  - [x] POC 阶段已在设备卡中展示短指纹占位和最后在线语义；真实公钥短指纹待配对身份接入后替换。
- [ ] 支持设备重命名、移除和空间密钥轮换。
