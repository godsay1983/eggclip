...
- [ ] DevicesPage：设备状态、重命名、移除。
  ...
  - [x] 新增 `TrustedDeviceStore`，从 active 同步空间读取已持久化的 trusted device；设备页展示显示名、公钥短指纹和最后在线时间，POC 运行时连接仍独立标为非可信。
...
- [ ] 生成 Ed25519 长期设备身份。
  ...
  - [x] 新增 `LocalDeviceIdentityService`，编排本机 `deviceId`、HUKS Ed25519 public key 和 private key ref 初始化；失败时不写入身份 metadata。
  - [x] `PairingStore` 已接入 `LocalDeviceIdentityService` 生成 CLIENT_HELLO 材料：握手 draft 只包含 deviceId、identity public key 和 ephemeral public key，不暴露 HUKS private key ref 或临时私钥。
...
- [ ] 显示已发送、待同步和失败状态。
  ...
  - [x] 首页已订阅 `PairingConnectionStore`，认证会话在线时明确显示正式加密同步；POC 状态不会覆盖该认证状态，认证失败会显示独立失败反馈。
...
- [ ] 局域网诊断：mDNS、候选地址、WebSocket 状态和错误码。
  ...
  - [x] 空间密钥自检已扩展为 AES-GCM 加解密与 HMAC-SHA-256 摘要别名联合校验；只显示通过/失败阶段，不展示摘要值或任何密钥材料。
