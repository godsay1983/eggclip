...
- HarmonyOS 设置页提供可选的 AppGallery 应用内评价入口，并仅在核心剪贴板操作成功后的低频里程碑检查自动请求。
...
- HarmonyOS：打开“设置 → 外观 → 语言”选择语言。保存并确认后选择“立即重启”，使应用名称、页面和权限说明统一使用所选语言；选择稍后或系统拒绝重启时，请从系统设置强行停止后重新打开。
